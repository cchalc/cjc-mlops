resource "databricks_group" "mlops-service-principal-group-staging" {
  display_name = "cjc_mlops-service-principals"
...
module "azure_create_sp" {
  ...
  service_principal_name       = "cjc_mlops-cicd"
  project_directory_path       = "/cjc_mlops"
  azure_tenant_id              = var.azure_tenant_id
  service_principal_group_name = "cjc_mlops-service-principals"
}
...
provider "databricks" {
  alias = "staging_sp"
  host  = "https://adb-4104328240019648.8.azuredatabricks.net"
  token = module.azure_create_sp.staging_service_principal_aad_token
}

provider "databricks" {
  alias = "prod_sp"
  host  = "https://adb-6801412720360756.16.azuredatabricks.net"
