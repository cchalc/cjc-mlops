...
def _get_ml_config_value(env, key):
    ...
    try:
        ...
        raise RuntimeError(
            ...
            f"cjc_mlops have been deployed to {env} (see cjc_mlops"
...
def get_model_name(env, test_mode=False):
    ...
    if env == "dev" or test_mode:
        ...
        return f"cjc_mlops-model{resource_name_suffix}"
    else:
        # Read ml model name from cjc_mlops/terraform
        return _get_ml_config_value(env, "cjc_mlops_model_name")
